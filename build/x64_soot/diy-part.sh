...
uci set network.lan.gateway='172.16.10.2'                     # 旁路由设置 IPv4 网关（去掉uci前面的#生效）
...
sed -i "s/OpenWrt /OpenWrt_x64_精简版 by GXNAS build $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt /g" "${ZZZ_PATH}"
