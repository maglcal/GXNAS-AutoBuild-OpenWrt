...
#添加lucky插件包
git clone https://github.com/sirpdboy/luci-app-lucky.git package/x64_gxnas-Software/lucky

#添加autoupdate插件包
git clone https://github.com/shidahuilang/luci-app-autoupdate.git package/x64_gxnas-Software/luci-app-autoupdate
...
uci set system.@system[0].hostname='OpenWrt-GXNAS'            # 修改主机名称为OpenWrt
...
sed -i "s/OpenWrt /OpenWrt_x64_测试版 by GXNAS build $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt /g" "${ZZZ_PATH}"
